...
// Error is near or identical to RemoveOnTouch for the pointing tasks.
...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public class HandDraggableError : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler
    {
        /// <summary>
	...
	private GameObject config;
        ...
        private void Update()
        {
            ...
            if (IsDraggingEnabled && isDragging) // cube is being tapped 
            {
		if (!some_error[error_itr]) // no error at first
		{
			UpdateDragging();
		}
		else
		{
			StartCoroutine(ErrorMethod());
		}
	...
	IEnumerator ErrorMethod() {
		IsDraggingEnabled = false; // disable call in update()

		if (some_error[error_itr] == true)
		{
			yield return new WaitForSeconds(1); // wait time 

			error_itr++;
			isDragging = false; // in case the user tries holding their tap on the cube 
			IsDraggingEnabled = true; // enable call in update()
		}
		else // no error
		{
			isDragging = false; // stops cube from moving when held
			IsDraggingEnabled = true; // enable call in update() 
		}
	...
	/* Shuffle error list */
	public void shuffleError (ref List<bool> error_collection)
	{
		...
		for (int i = 0; i < some_error.Count; i++)
		{
			random_placeholder = i + UnityEngine.Random.Range (0, error_collection.Count - i);

			/* Swap */
			temp = error_collection [i];
			error_collection [i] = error_collection [random_placeholder];
			error_collection [random_placeholder] = temp;
		}
